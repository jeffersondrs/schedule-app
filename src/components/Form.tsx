...
export default function Form() {
  ...
  const { name, phone, scheduleDescription, scheduleTime, scheduleDate } = state;
  const { scheduleStore } = useStore();

  const notify = () => toast.success("Appointment scheduled successfully!", {
    position: "top-center",
    autoClose: 3000,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col justify-center items-center gap-3 w-full z-50">
      ...
      <div className="flex flex-col justify-center items-start gap-1 w-full">
        <InputPhone phone={phone} setPhone={(phone) => handleChange("phone", phone)} />
      </div>

      <div className="flex flex-col justify-center items-start gap-1 w-full">
        <label htmlFor="service" className="text-xs font-bold text-gray-200">Description</label>
      ...
      <div className="flex flex-col justify-center items-start gap-1 w-full">
        <label htmlFor="scheduleDate" className="text-xs font-bold text-gray-200">Hour</label>
