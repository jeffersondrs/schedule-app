...
const DailyHeader: React.FC<{ title: SchedulePeriod }> = observer(
  ({ title }) => {
    const iconMap = {
      morning: <IoSunny className="w-5 h-5 text-blue-600" />,
      afternoon: <IoPartlySunnySharp className="w-5 h-5 text-orange-600" />,
      evening: <BsFillMoonStarsFill className="w-5 h-5 text-yellow-600" />,
    };
    ...
    return (
      <header className="flex flex-row justify-between items-center w-full border-gray-primary px-3 border-b flex-wrap sm:flex-nowrap">
        <div className="flex flex-row items-center gap-2 p-2">
          {iconMap[title]}
          <p className="tracking-wider text-white font-thin text-base capitalize">
            {title}
          </p>
        </div>
